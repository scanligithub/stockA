...
# 保守并发：限制为 2，避免服务器断开连接
sem = asyncio.Semaphore(2)

async def fetch_page(page_no, pz, url, params, headers, retries=3):
    """每次请求都创建新会话，避免连接复用导致的断开"""
    async with sem:
                ...
                async with aiohttp.ClientSession(headers=headers, connector=conn) as new_session:
                    async with new_session.get(url, params=page_params, timeout=60) as resp:
                        if resp.status == 200:
                            ...
                            print(f"[!] 403 被拒，深度休眠 45s...")
                            await asyncio.sleep(45)
                            continue

                # 如果响应不是 200 或 403，继续重试
...
async def fetch_all_a_shares():
    ...
    for i in range(1, total_pages + 1):
        ...
        page_data = await fetch_page(i, page_size, API_URL, base_params, headers)
        if page_data:
