...
def main():
    ...
    if k_files: 
        con.execute(f"CREATE OR REPLACE VIEW v_initial AS SELECT DISTINCT code FROM read_parquet({k_files})")
    ...
    if not df_repaired.empty:
        ...
        print(f"💎 Repair complete. Total K-Line files: {len(k_files)}")
